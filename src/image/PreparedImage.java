...
public class PreparedImage {
    ...
    public PreparedImage(Mat image) {
        this.image = image;
    }
    ...
    public Mat prepare(){
        ...
        Imgcodecs.imwrite("binarized-cropped.jpg", image);//for debug
        return image;
    }

    private Mat binarize(Mat source){
        Mat gray = new Mat();
        ...
        Imgproc.adaptiveThreshold(gray, binary, 255, ADAPTIVE_THRESH_GAUSSIAN_C, THRESH_BINARY,
                PIXEL_NEIGHBOURHOOD, SUBTRACTED_FROM_MEAN);
        return binary;
    }

    private Mat binarizeAndThickenElements(Mat source){
        Mat binary = binarize(source);
        Mat eroded = new Mat();
